 ...
 * Created by chenl
 */
public class CreatChatRoom2TreeView extends ListView implements OnItemClickListener{
    ...
    //用于存储要删除的子节点，以及间接子节点。
    CreatChatRoom2TreeViewAdapter adapter = null;// 用于数据填充
    LastLevelItemClickListener itemClickCallBack;// 用户点击事件回调
    ParentLevelItemClickListener pItemClickCallBack;//用户点击父节点事件回调----------------cl
  
    public CreatChatRoom2TreeView(final Context context, AttributeSet attrs) {
       super(context, attrs);
       ...
       adapter = new CreatChatRoom2TreeViewAdapter(context, currentElements);
