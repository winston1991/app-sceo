...
public class GestureDrawline extends View {
	...
	public GestureDrawline(Context context, List<GesturePoint> list, boolean isVerify,
		...
		paint.setStrokeWidth(30);//笔宽5像素
		paint.setColor(getResources().getColor(R.color.gesture_blue));
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (currentPoint != null) {
				currentPoint.setPointState(SceoUtil.POINT_STATE_SELECTED);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (pointAt == null || currentPoint.equals(pointAt) || SceoUtil.POINT_STATE_SELECTED == pointAt.getPointState()) {
				//点击移动区域不在圆的区域，或者当前点击的点与当前移动到的点得位置相同，或者当前点击的点处于选中状态
				...
				pointAt.setPointState(SceoUtil.POINT_STATE_SELECTED);
				
				//判断是否中间点需要选中
				GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint, pointAt);
				if (betweenPoint != null && SceoUtil.POINT_STATE_SELECTED != betweenPoint.getPointState()) {
					//存在中间点并且没有被选中
					...
					//设置中间点选中
					betweenPoint.setPointState(SceoUtil.POINT_STATE_SELECTED);
					//赋值当前的point
					currentPoint = pointAt;
				}else {
			...
			invalidate();
			break;
		case MotionEvent.ACTION_UP://当手抬起时
			if (isVerify) {
				//手势密码校验
	...
	public void clearDrawlineState(long delayTime){
		...
		new Handler().postDelayed(new clearStateRunnble(), delayTime);
	}
	
	/**
	 * 清除绘制状态的线程
	 */
	final class clearStateRunnble implements Runnable{
		@Override
		public void run() {
			//重置passWordSb
			...
			for (GesturePoint p : list) {
				p.setPointState(SceoUtil.POINT_STATE_NORMAL);
			}
			invalidate();
			isDrawEnable = true;
		}
	...
	private GesturePoint getPointAt(int x, int y){
		for (GesturePoint point : list) {
			...
			//如果执行到这，说明当前点击的点得位置在遍历到点的位置的这个地方
			return point;
		}
		return null;
	}
	...
	private void drawErrorPathTip(){
		canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
		paint.setColor(getResources().getColor(R.color.gesture_red));
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
			pair.first.setPointState(SceoUtil.POINT_STATE_WRONG);
			pair.second.setPointState(SceoUtil.POINT_STATE_WRONG);
			canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(), 
					pair.second.getCenterX(), pair.second.getCenterY(), paint);//画线
		}
		invalidate();
	...
	public interface GestureCallBack{
		 ...
		 * 代表用户绘制的密码与传入的密码不想同
		 */
		public abstract void checkedFailed();
	}
