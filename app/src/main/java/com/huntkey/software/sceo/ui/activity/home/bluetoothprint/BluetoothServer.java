...
import com.huntkey.software.sceo.bean.eventbus.BtMsgReadEvent;
...
public class BluetoothServer {
    ...
    private synchronized void setState(int state) {
        ...
        switch (mState) {
            case STATE_LISTEN:
                EventBusUtil.getInstence().post(new PrintMsgEvent(1, "等待连接"));
                break;
            case STATE_CONNECTING:
                EventBusUtil.getInstence().post(new PrintMsgEvent(1, "正在连接"));
                break;
            case STATE_CONNECTED:
                EventBusUtil.getInstence().post(new PrintMsgEvent(1, "已连接"));
                break;
            default:
                EventBusUtil.getInstence().post(new PrintMsgEvent(1, "未连接"));
                break;
        }
...
//        EventBusUtil.getInstence().post(new PrintMsgEvent(2, "正在连接蓝牙设备"));
    ...
    public synchronized void connected(BluetoothSocket socket,
        ...
        EventBusUtil.getInstence().post(new PrintMsgEvent(2, "蓝牙设备连接成功"));
...
//         Send a failure message back to the Activity
        EventBusUtil.getInstence().post(new PrintMsgEvent(2, "蓝牙连接失败,请重启打印机再试"));
        setState(STATE_NONE);
...
//         Send a failure message back to the Activity
        EventBusUtil.getInstence().post(new PrintMsgEvent(2, "蓝牙连接断开"));
        setState(STATE_NONE);
        // Start the service over to restart listening mode
        BluetoothServer.this.start();
    }
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    // send data
                    EventBusUtil.getInstence().post(new BtMsgReadEvent(bytes, buffer));
                } catch (IOException e) {
                    e.printStackTrace();
                    connectionLost();
                    // Start the service over to restart listening mode
